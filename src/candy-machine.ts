...
import { COLLECTION_NAME, UPDATE_AUTHORITY, SYMBOL, VERIFIED_ADDRESS } from "./constant";
import {
  CIVIC,
  getAtaForMint,
  getNetworkExpire,
  ...
  for (let index = 0; index < tokenAccounts.value.length; index++) {
    try{
      ...
      if (tokenAmount.amount == "1" && tokenAmount.decimals == "0") {
        let mint = new anchor.web3.PublicKey(tokenAccount.account.data.parsed.info.mint)
        ...
        // const checkCreator = (creator : any) =>{
        //   return creator.address == VERIFIED_ADDRESS
        // }
        // let finded = metadata.data.data.creators.find(checkCreator)
        // if(finded != undefined){
          const entireData = { ...data, id: Number(data.name.replace( /^\D+/g, '').split(' - ')[0]) }
          allTokens.push({ metadata : metadata.data.data, offChainData : entireData, mint : metadata.data.mint, address : pda })
        }
      }
